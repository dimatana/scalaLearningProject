...
class ErrorMappingSpec extends FunSuite:
  ...
  test("InvalidOdds maps to 422"):
    assertEquals(ErrorMapping.toStatus(BetError.InvalidOdds("odds must be greater than 1.0")), Status.UnprocessableEntity)

  test("RepositoryFailure maps to 500"):
    assertEquals(ErrorMapping.toStatus(BetError.RepositoryFailure(new RuntimeException("db timeout"))), Status.InternalServerError)